...
def get_args():
    ...
    parser.add_argument("--buffer-max-train-size", type=float, default=0.)
...
def test_discrete_cql(args=get_args()):
    ...
    elif os.path.isdir(args.load_buffer_name):
        buffer = VectorReplayBuffer.load_hdf5(args.load_buffer_name)
    else:
        print(f"Unknown buffer format: {args.load_buffer_name}")
        exit(0)
    
    if args.buffer_max_train_size != 0:
        buffer.maxsize = args.buffer_max_train_size
    ...
    def watch():
        ...
        result = test_collector.collect(n_episode=args.test_episodes, render=args.render)
